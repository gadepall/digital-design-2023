...
\begin{table}[!h]
\centering
\input{./stm32/setup/tables/connections.tex}
...
\item To make the stm32 board into Programming mode, by setting the alligners to programming mode as shown in Fig \ref{fig:programming_mode}. This configuration will ensure that the microcontroller enters the system memory programmer on reset.
\begin{figure}[!h]
\begin{center}
\includegraphics[width=\columnwidth]{./stm32/setup/figs/programming_mode.png}
\end{center}
...
\item After flashing, the inbuilt LED present on the stm32f103c8 will start blinking and then change the STM32 board to operating mode as shown in Fig \ref{fig:programming_mode} inorder run your code from the main flash memory.
\end{enumerate}
